...
def main():
    ...
    # plot_years_and_average(by_year_data, days_of_year)
    plot_specific_year(by_year_data, 2021)
...
def plot_years_and_average(by_year_data, days_of_year):
...
def plot_specific_year(year_data, year):
    data = year_data[year]
    fig, ax = plt.subplots(1,1,figsize=(9,3),dpi=96)
    ax.plot(data['dates'],data['distances'])
    apply_common_styles(ax, data, year, include_xaxis=True)
    ax.set_title(f"Distance from Home ({year})")
    y_max_miles = 2500
    y_max_km = y_max_miles * 1.609
    ...
    for loc_date, city in location_dates.items():
        ...
        # ax.text(data['dates'][day], data['distances'][day], city, **style)
        ax.annotate(city,
