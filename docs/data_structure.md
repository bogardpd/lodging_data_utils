...
Although the layer geometry is Multipoint for potential future capabilities, each feature should have only one point. In locations with multiple buildings, this point should be set to the building with the check-in desk (if one exists) or the most significant building of the location (if there is no check-in desk).
